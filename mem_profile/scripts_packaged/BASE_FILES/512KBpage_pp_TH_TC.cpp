...
int process_phase(){
	...
	uint64_t TC_mem_footprint = 50000000000;
	uint64_t pool_cap = TC_mem_footprint / PAGESIZE;
	pool_cap = pool_cap / POOL_FRACTION;
	cout<<"pool cap: "<<pool_cap<<"(="<<pool_cap*PAGESIZE/(1024*1024)<<"MB)"<<endl;
	...
	while((i<MIGRATION_LIMIT) || (i_cxi<MIGRATION_LIMIT)){
		...
		if(accesses < migration_threshold){break;}
	...
	if(((migrated_pages_CI+pages_to_CI)*2) < MIGRATION_LIMIT ){ //decrease limit if too few pages got migrated
