...
uint64_t migration_threshold = MIGRATION_THRESHOLD;
uint64_t eviction_threshold =1000;


int process_phase(){
	bool not_enough_eviction_candidates=false;
//	bool not_enough_migration_candidates=true;
	...
	std::multiset<std::pair<uint64_t, uint64_t>, migration_compare> sorted_candidates_pseudoshuffle;
...
#else
	//else{
		uint64_t tmpth=migration_threshold;
		uint64_t tmpcount = std::count_if(sorted_candidates.begin(), sorted_candidates.end(),
								[tmpth](const std::pair<uint64_t, uint64_t>& element) {
                                  return element.second > tmpth;
                              });
		//get the ratio of pages larger than (lt) TH OVER MIGRATION_LIMIT. if R is larger than 1, add 1/R pages to the sorted_candidates list
		uint64_t ratio_of_pages_lt_TH= tmpcount / (MIGRATION_LIMIT*(curphase+1));
		if(ratio_of_pages_lt_TH<1){ratio_of_pages_lt_TH=1;}
		...
		for (const auto& ppair : page_access_counts_sampled_joined){
			...
			if(tmpi%ratio_of_pages_lt_TH==0){
				sorted_candidates_pseudoshuffle.insert({page, accs});
			}
			tmpi++;
		}
	...
	save_page_owners(page_owner,"page_owner.txt\0");
	save_page_owners(page_owner_CI,"page_owner_CI.txt\0");


	//generate shuffled list
	std::multiset<std::pair<uint64_t, uint64_t>, migration_compare> sorted_candidates_copy(sorted_candidates);
	std::vector<std::pair<uint64_t, uint64_t>> temp_vector(sorted_candidates_copy.begin(), sorted_candidates_copy.end());
	 // Step 2: Shuffle the vector
    std::random_device rd;
    std::mt19937 g(rd());
    std::shuffle(temp_vector.begin(), temp_vector.end(), g);

    // Step 3: Construct a new multiset from the shuffled vector
    //std::multiset<std::pair<uint64_t, uint64_t>, migration_compare> shuffled_candidates(temp_vector.begin(), temp_vector.end());
    std::multiset<std::pair<uint64_t, uint64_t>> shuffled_candidates(temp_vector.begin(), temp_vector.end());
	...
	auto it_migration = sorted_candidates_pseudoshuffle.begin();
	auto eviction_it = shuffled_candidates.rbegin();
	...
	while((i<MIGRATION_LIMIT) || (i_cxi<MIGRATION_LIMIT)){
		...
		U64 accesses = it_migration->second;
		if(accesses < migration_threshold){
			//not_enough_migration_candidates=true;
			break;
		}
				...
				if(!eviction_candidate_was_hot && (sharers >= SHARER_THRESHOLD)){
					if (old_owner != CXO) {
						...
						if(pool_cap <= pages_in_pool){
							...
							for(; eviction_it != shuffled_candidates.rend(); ++eviction_it) {
								if((page_owner_CI[eviction_it->first] == CXO) &&(eviction_it->second < eviction_threshold )) {
									evicted_page = eviction_it->first;
									break;
								}
							}
							//std::cout<<"eviction_candidate access count: "<<eviction_it->second<<endl;
							if(eviction_it==shuffled_candidates.rend()) {
								...
								not_enough_eviction_candidates=true;
							}
							//U64 ev_most_acc=0;
		...
		it_migration++;
		if(it_migration==sorted_candidates_pseudoshuffle.end()){//reached end of pages
			cout<<"reached end of sorted_candidates"<<endl;
			cout<<"sorted candidates size: "<<sorted_candidates.size()<<endl;
			cout<<"migrated pages in StarNUMA: "<<migrated_pages_CI+pages_to_CI<<endl;
			break;
		}
	...
	std::cout<<"reassign owners done, migration_threshold: "<<migration_threshold<<std::endl;
	std::cout<<"migration_limit: "<<MIGRATION_LIMIT<<", number of StarNUMA migration: "<<migrated_pages_CI+pages_to_CI <<std::endl;

	if(not_enough_eviction_candidates){
		eviction_threshold = eviction_threshold*2;
	}
	else if(((migrated_pages_CI+pages_to_CI)*2) < MIGRATION_LIMIT ){ //decrease limit if too few pages got migrated
		migration_threshold=migration_threshold*3/4;
		std::cout<<"decrease migration_threshold: "<<migration_threshold<<std::endl;
	}
	//TODO else if number of pages that had more accesses than threshold but didn't get migrated is > MIGRATION_LIMIT, double the TH
	else{ // increase migration limit if too many pages have accesses larger than TH
		uint64_t tmpth=migration_threshold;
		uint64_t tmpcount = std::count_if(sorted_candidates.begin(), sorted_candidates.end(),[tmpth](const std::pair<uint64_t, uint64_t>& element) {
                                  return element.second > tmpth;
                              });
		 if(tmpcount > (MIGRATION_LIMIT*2)){
			migration_threshold=migration_threshold*2;
			std::cout<<"increased migration_threshold: "<<migration_threshold<<std::endl;
		 }
		 //adjust eviction limit
		 tmpth = eviction_threshold;
		 tmpcount = std::count_if(sorted_candidates.begin(), sorted_candidates.end(),[tmpth](const std::pair<uint64_t, uint64_t>& element) {
                                  return element.second < tmpth;
                              });
		 if(tmpcount > (MIGRATION_LIMIT*2)){
			eviction_threshold=eviction_threshold*3/2;
			std::cout<<"increased eviction_threshold: "<<eviction_threshold<<std::endl;
		 }
