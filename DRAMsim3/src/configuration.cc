...
Address Config::AddressMapping(uint64_t hex_addr) const {
	...
	if(ch_pos==-1){ //5channel case (10 logical channel)
    	int channel = (hex_addr % 10);
		//std::cout<<"channel: "<<channel<<std::endl;
		hex_addr = hex_addr / 10;
...
void Config::SetAddressMapping() {
	...
	std::cout<<"fieldwidths_channel: "<<field_widths["ch"]<<std::endl;
	...
	if(address_mapping.size()==14 ){ //5channel case (10 logical channel)
    	std::vector<std::string> fields;
    	for (size_t i = 0; i < address_mapping.size()-4; i += 2) {
    	...
    	//ch_pos = field_pos.at("ch"); handle channel differently
		ch_pos=-1;
		...
		std::cout<<"dbg:bg_pos "<<bg_pos<<std::endl;
		std::cout<<"dbg:ba_pos "<<ba_pos<<std::endl;
