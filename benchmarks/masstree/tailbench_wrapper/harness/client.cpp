...
Client::Client(int _nthreads) {
	...
	lambda = getOpt<double>("TBENCH_QPS", 1000000.0) * 1e-9;
    //lambda = getOpt<double>("TBENCH_QPS", 10.0) * 1e-9;
...
Request* Client::startReq() {
    ...
    if (curNs < req->genNs) {
		printf("clinet calling ns sleep");
...
void Client::dumpStats() {
	pid_t lats_pid=getpid();
	std::string name_string="lats";
	name_string += std::to_string(lats_pid);
	name_string += ".bin";
    //std::ofstream out("lats.bin", std::ios::out | std::ios::binary);
    std::ofstream out(name_string, std::ios::out | std::ios::binary);
