...
pvector<NodeID> Afforest(const Graph &g, int32_t neighbor_rounds = 30) {
  pvector<NodeID> comp(g.num_nodes());
	std::cout<<"starting Afforest"<<std::endl;
  //lets just trace the full function
  	uint64_t dummyval=0;
    ...
    __asm__ __volatile__(
        "xchg %%rbx, %%rbx;"
        ::"b"((uint64_t)(&dummyval)),"c"((uint64_t)0x2) //clobbered registers
    );
    __asm__ __volatile__(
        "xchg %%rbx, %%rbx;"
...
int main(int argc, char* argv[]) {
	std::cout<<"running cc mian"<<std::endl;
