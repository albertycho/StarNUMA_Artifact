...
tbcpu_fmtstr='TB_CPU {name}({index},{frequency}, &ICN);\n'
...
define_icnstr = '#define {{names[{name}]}} 2.4*{{config[{name}]}}ul\n'
...
default_core = { 'frequency' : 2400, 'ifetch_buffer_size': 64, 'decode_buffer_size': 32, 'dispatch_buffer_size': 32, 'rob_size': 352, 'lq_size': 128, 'sq_size': 72, 'fetch_width' : 6, 'decode_width' : 6, 'dispatch_width' : 6, 'execute_width' : 4, 'lq_width' : 2, 'sq_width' : 2, 'retire_width' : 5, 'mispredict_penalty' : 1, 'scheduler_size' : 128, 'decode_latency' : 1, 'dispatch_latency' : 1, 'schedule_latency' : 0, 'execute_latency' : 0, 'branch_predictor': 'bimodal', 'btb': 'basic_btb' }
...
default_pmem = { 'name': 'DRAM', 'frequency': 3200, 'cxl_mode': 0, 'channels': 1, 'ranks': 1, 'banks': 8, 'rows': 65536, 'columns': 128, 'lines_per_column': 8, 'channel_width': 8, 'wq_size': 64, 'rq_size': 64, 'tRP': 12.5, 'tRCD': 12.5, 'tCAS': 12.5, 'tCXL': 30, 'turn_around_time': 7.5 }
default_vmem = { 'size': 8589934592, 'num_levels': 5, 'minor_fault_penalty': 200 }
default_icn  = {'name':'ICN', 'frequency': 2400, "t1hop" : 50, "t2hop" : 280, "t_cxl" : 120, "bw_upi_in_MB" : 3000, "bw_cxl_in_MB" : 10000, 'lower_level':'DRAM'}
default_tbcpu= {'frequency' : 2400, 'num_tb_cores' : 1}
default_migrator= {'enable' : False}
...
config_file['icn_sim'] = ChainMap(config_file['icn_sim'], default_icn.copy())
config_file['tb_cpu'] = ChainMap(config_file['tb_cpu'], default_tbcpu.copy())
config_file['migrator'] = ChainMap(config_file['migrator'], default_migrator.copy())
#icn_sim=config_file.get('ICN_SIM',{})
config_file['LLC']['lower_level'] = 'ICN'
default_llc['lower_level'] = 'ICN'

tb_cpu_names = []
for i in range(config_file['tb_cpu']['num_tb_cores']):
    tb_cpu_names.append('tbcpu'+str(i))
...
# Scale frequencies
##save icnfreq mult here, for later use
orig_icn_freq=config_file['icn_sim']['frequency']
icnfreq_multiplier=config_file['icn_sim']['frequency']
icnfreq_multiplier=icnfreq_multiplier/1000
config_file['physical_memory']['io_freq'] = config_file['physical_memory']['frequency'] # Save value
config_file['icn_sim']['io_freq'] = config_file['icn_sim']['frequency'] # Save value
freqs = list(itertools.chain(
    [cpu['frequency'] for cpu in cores],
    [cache['frequency'] for cache in caches.values()],
    (config_file['icn_sim']['frequency'],),
    (config_file['physical_memory']['frequency'],)
))
freqs = [max(freqs)/x for x in freqs]
for freq,src in zip(freqs, itertools.chain(cores, caches.values(), (config_file['icn_sim'],), (config_file['physical_memory'],))):
    src['frequency'] = freq
...
for fill_level in range(1,len(memory_system)+1):
    for k in active_keys:
        if memory_system[k]['lower_level'] != 'DRAM' and memory_system[k]['lower_level'] != 'ICN':
    ...
    active_keys = [memory_system[k]['lower_level'] for k in active_keys if memory_system[k]['lower_level'] != 'DRAM' and memory_system[k]['lower_level'] != 'ICN']
...
for i in reversed(range(len(memory_system))):
    ul = memory_system[i]
    if ul['lower_level'] != 'DRAM' and ul['lower_level'] != 'ICN':
...
with open(instantiation_file_name, 'wt') as wfp:
    ...
    wfp.write('#include "hybrid_mem_wrapper.hpp"\n')
    wfp.write('#include "icn_sim.h"\n')
    wfp.write('#include "migrator.h"\n')
    wfp.write('#include "ooo_cpu.h"\n')
    wfp.write('#include "tb_cpu.h"\n')
    ...
    wfp.write(icn_sim_fmtstr.format(attrs=config_file['icn_sim']))
    for elem in memory_system:
    ...
    tbcpu_index=0
    for tbcpu_name in tb_cpu_names:
        tbcpu_tmp={}
        tbcpu_tmp['name']=tbcpu_name
        tbcpu_tmp['frequency']=cores[0]['frequency']
        tbcpu_tmp['index']=tbcpu_index
        tbcpu_index = tbcpu_index+1
        wfp.write(tbcpu_fmtstr.format(**tbcpu_tmp))

    if (config_file['migrator']['enable']==True):
        wfp.write(migrator_fmtstr)
    ...
    if(config_file['tb_cpu']['num_tb_cores']>0):
        wfp.write('std::array<TB_CPU*, NUM_TBCPU> tbcpus {\n')
        iiiii=0
        for tbcpu_name in tb_cpu_names:
            if(iiiii==0):
                wfp.write('&'+tbcpu_name)
                iiiii=1;
            else:
                wfp.write(', &'+tbcpu_name)
        wfp.write('\n};\n')
    ...
    wfp.write(', '.join('&{name}'.format(**elem) for elem in itertools.chain(cores, memory_system, (config_file['icn_sim'],), (config_file['physical_memory'],))))
    wfp.write('\n')
    for tbcpu_name in tb_cpu_names:
        wfp.write(', &'+tbcpu_name)
    if (config_file['migrator']['enable']==True):
...
with open(constants_header_name, 'wt') as wfp:
    ...
    wfp.write('#define NUM_TBCPU ' + str(config_file['tb_cpu']['num_tb_cores']) + 'u\n')
    # NUM_OPERABLES: +1 for dram, +1 for ICN, +1 if migrator
    if (config_file['migrator']['enable']==True):
        wfp.write('#define NUM_OPERABLES ' + str(len(cores) + len(memory_system) + 1+1+config_file['tb_cpu']['num_tb_cores'] +1) + 'u\n')
    else:
        wfp.write('#define NUM_OPERABLES ' + str(len(cores) + len(memory_system) + 1+1+config_file['tb_cpu']['num_tb_cores']) + 'u\n')

    for k in const_names['physical_memory']:
        if k in ['tRP', 'tRCD', 'tCAS', 'tCXL', 'turn_around_time']:
    ...
    ## write icn constants
    #print(icnfreq_multiplier)
    wfp.write('#define T_LOCAL 1\n')
    wfp.write('#define T_1HOP '+str(int((icnfreq_multiplier/2)*config_file['icn_sim']['t1hop']))+'\n')
    wfp.write('#define T_2HOP '+str(int((icnfreq_multiplier/2)*config_file['icn_sim']['t2hop']))+'\n')
    wfp.write('#define T_CXL '+str(int((icnfreq_multiplier/2)*config_file['icn_sim']['t_cxl']))+'\n')
    wfp.write('#define UPI_BW_INTERVAL '+str(int((64*orig_icn_freq)/config_file['icn_sim']['bw_upi_in_MB']))+'\n')
    wfp.write('#define CXL_BW_INTERVAL '+str(int((64*orig_icn_freq)/config_file['icn_sim']['bw_cxl_in_MB']))+'\n')
    wfp.write('#define CXO 100\n')
    wfp.write('#define N_SOCKETS '+str(config_file['icn_sim']['n_sockets'])+'\n')
    #wfp.write('#define CXL_BIT '+str(config_file['physical_memory']['CXL_BIT'])+'\n')
    # for k in const_names['icn_sim']:
    #     # if 't1hop' in k:
    #     #     ns_to_cycle=
    #     #     wfp.write('#define T_1HOP '+)
    #     if k in ['t1hop','t2hop','t_cxl']:
    #         wfp.write(define_icnstr.format(name=k).format(names=const_names['icn_sim'], config=config_file['icn_sim']))
