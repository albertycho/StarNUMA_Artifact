...
public:
  ...
  //const static uint64_t tCXL = detail::ceil(1.0 * tCXL_DRAM_NANOSECONDS * DRAM_IO_FREQ / 1000);
  ...
  MEMORY_CONTROLLER(double freq_scale) : champsim::operable(freq_scale), MemoryRequestConsumer(std::numeric_limits<unsigned>::max()) {
    //std::cout << "MEMORY CONTROLLER tRP " << tRP << " tRCD " << tRCD << " tCAS " << tCAS 
    //          << " tCXL " << tCXL << " CXL_MODE " << CXL_MODE << std::endl; 
  }
