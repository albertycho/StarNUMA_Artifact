...
#define COUNTER_BASE_ADDR 100000

extern ICN_SIM ICN;

extern VirtualMemory vmem;
extern uint8_t warmup_complete[NUM_CPUS];
extern uint64_t n_completed_translation[NUM_CPUS];
...
//helper function to send counter update on TLB miss
int send_counter_update(uint64_t paddr, uint64_t srccpu, uint64_t cur_cycle){
  // send 16 counter updates per miss - 
  //  since we only have 1 socket with TLB modeling, 
  //  assume all other sockets access the counter as many times as socket 0
  PACKET wpackets[N_SOCKETS];
  for(uint64_t i=0; i<N_SOCKETS;i++){
    PACKET &wpacket = wpackets[i];
    uint64_t counter_offset = (paddr>>LOG2_PAGE_SIZE)+128*i;
    uint64_t counter_addr = COUNTER_BASE_ADDR+counter_offset; // why does it give me warning..?
    wpacket.address=counter_addr;
    wpacket.cpu = srccpu+(i*NUM_CPUS);
    wpacket.type=WRITEBACK;
    wpacket.to_return={};
    wpacket.instr_id=333333; // doesn't matter
    wpacket.cycle_enqueued=cur_cycle;
    int retval = ICN.add_wq(&wpacket);
  ...
  while (fill_this_cycle > 0 && !std::empty(MSHR) && MSHR.front().event_cycle <= current_cycle) {
    ...
    if (fill_mshr->translation_level == 0) // If translation complete
    {
      ...
      if (warmup_complete[cpu] && fault) {
        ...
        //TODO: To model traffic from counter update, could send a memory request to socket 0
        // lets just count these for now
        n_completed_translation[cpu]++;
        send_counter_update(addr,cpu,current_cycle);
...
int PageTableWalker::add_rq(PACKET* packet)
{
  ...
  // if(packet->address == 0){
	//   std::cout<<"ptw.cc Warning - address ==0 detected"<<std::endl;
  //   packet->address = 100;
  // }
