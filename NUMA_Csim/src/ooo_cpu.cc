...
void O3_CPU::operate()
{
  ...
  //   coh_dir.update_block(654321, cpu, false);
  // }

  // we don't stall the core anymore, but keep the variable for pacing the migrations
  if(TLB_shootdown_stall[cpu]>0){
    TLB_shootdown_stall[cpu]=TLB_shootdown_stall[cpu]-1;
    TLB_sd_stalls++;

    //commented out - no more cpu stalls
    //return;
  }
...
void O3_CPU::do_fetch_instruction(champsim::circular_buffer<ooo_model_instr>::iterator begin, champsim::circular_buffer<ooo_model_instr>::iterator end)
{
  ...
  if (rq_index != -2) {
    coh_dir.update_block(fetch_packet.address, cpu, true);
      ...
      if (dependent->source_memory[j] && dependent->source_added[j]) {
        if (dependent->source_memory[j] == sq_it->virtual_address) { // this is required since a single
          ...
          if(dependent->lq_index[j]->producer_id != sq_it->instr_id){
            ...
            std::cout<<sq_it->instr_id<<std::endl;
          }
...
int O3_CPU::execute_load(std::vector<LSQ_ENTRY>::iterator lq_it)
{
  ...
  #if ENABLE_COHERENCE
  int block_owner = coh_dir.get_coh_owner(data_packet.address, cpu, false);
  if(block_owner!=-1){
    //DBG so far, code shouldn't get here, TODO -remove print eventually
    //std::cout<<"just testing single socket, so should not get here"<<endl;
    //std::cout<<"owner: "<<block_owner<<endl;

    assert(block_owner<N_SOCKETS);
    data_packet.block_transfer=true;
    data_packet.block_owner=block_owner;
  }
  #endif
  ...
  while (retire_bandwidth > 0 && !ROB.empty() && (ROB.front().executed == COMPLETED)) {
    for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
      if (ROB.front().destination_memory[i]) {
        ...
        #if ENABLE_COHERENCE
        int block_owner = coh_dir.get_coh_owner(data_packet.address, cpu, true);
        if(block_owner!=-1){
          assert(block_owner<N_SOCKETS);
          data_packet.block_transfer=true;
          data_packet.block_owner=block_owner;
...
void O3_CPU::print_deadlock()
{
  ...
  if (!std::empty(ROB)) {
    ...
    std::cout << " time in ROB without action: " << current_cycle - ROB.front().event_cycle;
    std::cout <<" ROB Occupancy: "<< ROB.occupancy()<< "/"<<ROB.size()<<std::endl;
    std::cout << std::endl;
  } else {
  ...
  for (auto lq_it = std::begin(LQ); lq_it != std::end(LQ); ++lq_it) {
    if (is_valid<LSQ_ENTRY>{}(*lq_it))
      std::cout << "[LQ] entry: " << std::distance(std::begin(LQ), lq_it) << " instr_id: " << lq_it->instr_id << " address: " << std::hex
                << lq_it->physical_address << std::dec << " translated: " << +lq_it->translated << " fetched: " << +lq_it->fetched<< "time in lq without action: "<<current_cycle - lq_it->event_cycle << std::endl;
  }
  ...
  for (auto sq_it = std::begin(SQ); sq_it != std::end(SQ); ++sq_it) {
    if (is_valid<LSQ_ENTRY>{}(*sq_it))
      std::cout << "[SQ] entry: " << std::distance(std::begin(SQ), sq_it) << " instr_id: " << sq_it->instr_id << " address: " << std::hex
                << sq_it->physical_address << std::dec << " translated: " << +sq_it->translated << " fetched: " << +sq_it->fetched<< "time in sq without action: "<<current_cycle - sq_it->event_cycle << std::endl;
  }
}
